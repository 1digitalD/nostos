...
def test_search_context_warns_when_profile_area_key_weight_missing_from_citypack(
    tmp_path: Path,
) -> None:
    citypack_path = tmp_path / "citypack.yaml"
    profile_path = tmp_path / "profile.yaml"
    ...
    _write_yaml(
        profile_path,
        {
            ...
            "hard": {"exclude": []},
            "weights": {"laundry.in_suite": 6},
            "area_key_weights": {"downtown_van": 8},
            "sources": {"craigslist": True},
            "notify": [],
            "schedule": "0 */6 * * *",
        },
    ...
    with pytest.warns(
        UserWarning,
        match=r"area_key_weights.*citypack.*downtown_van",
    ):
        context = load_search_context(citypack_path=citypack_path, profile_path=profile_path)
